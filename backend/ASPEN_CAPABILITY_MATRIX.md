...
| **Parallel / large-scale throughput** | Multi-process worker pool (`parallel_evaluator`, `bridge.parallel_evaluate_designs`) — N CLRs evaluate a batch concurrently | A *single* solve is still serial (so is Aspen's); validated 1.9×/4 workers on population batches |
