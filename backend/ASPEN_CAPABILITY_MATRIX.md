...
| **Infeasible-path optimization** (simultaneous tear + optimize) | `run_infeasible_path_optimizer` — tear variables as decision vars, closure as equality constraints (Biegler) | **Validated** (exact optimum, 12.8× fewer passes). Live DWSIM coupling attempted: correct mechanism is an open-loop build (tear as fixed feed), but reading back the computed tear stream is blocked — **live-pending** |
